...
function tipPerPerson(bill, people, tip, typeOfTip) {
  if (typeOfTip == "custom") {
    document.getElementById("tip").innerText =
      "$" + (Number(tip) / people).toFixed(2);
    document.getElementById("total").innerText =
      "$" + ((Number(bill) + Number(tip)) / people).toFixed(2);
  } else {
    document.getElementById("tip").innerText =
      "$" + ((Number(bill) * Number(tip)) / 100 / people).toFixed(2);
    document.getElementById("total").innerText =
      "$" + ((Number(bill) * (Number(tip) / 100 + 1)) / people).toFixed(2);
  }
}
...
validateBill = () => {
  ...
  if ((bill.match(/\./g) || []).length > 1) {
    bill =
      bill.split(".").slice(0, 1).join("") +
      "." +
      bill.split(".").slice(1).join("");
  }
  if ((bill.match(/\./g) || []).length == 1) {
    bill =
      bill.split(".")[0] +
      "." +
      bill.split(".")[1].split("").slice(0, 2).join("");
  }
...
function validateCustomTip() {
  ...
  if ((customTip.match(/\./g) || []).length > 1) {
    customTip =
      customTip.split(".").slice(0, 1).join("") +
      "." +
      customTip.split(".").slice(1).join("");
  }
  if ((customTip.match(/\./g) || []).length == 1) {
    customTip =
      customTip.split(".")[0] +
      "." +
      customTip.split(".")[1].split("").slice(0, 2).join("");
  }
